...
def signupPage(request):
    if request.method == "POST":
        ...
        con_pass_word = request.POST.get('confirmpassword')
        ...
        user.email = mail
        user.confirm_password = con_pass_word
...
def add_job_Page(request):
    ...
    if request.method == 'POST':
        ...
        Number_of_openings=request.POST.get('numberofopenings')
...
def matching_jobs(request):
    ...
    matching_jobs = job_model.objects.filter()

    context = {
        'matching_jobs': matching_jobs
    }
    return render(request, 'matching_jobs.html', context)
